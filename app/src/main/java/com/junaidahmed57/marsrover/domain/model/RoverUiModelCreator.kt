package com.junaidahmed57.marsrover.domain.model

import com.junaidahmed57.marsrover.R

val roverUiModelList = listOf(
    RoverUiModel("Perseverance", R.drawable.perseverance, landingDate = "18 February 2021", distance = "12.56 km"),
    RoverUiModel("Opportunity", R.drawable.opportunity, landingDate = "25 January 2004", distance = "45.16 km"),
    RoverUiModel("Spirit", R.drawable.spirit, landingDate = "04 January 2004", distance = "7.73 km"),
    RoverUiModel("Curiosity", R.drawable.curiosity, landingDate = "06 August 2012", distance = "29.27 km"),
)